-- 마이그레이션: 근로자, 스케줄, 파일 관리 스키마 업데이트 (v3)
-- 날짜: 2025-01-21
-- 설명: auth.users 테이블 사용, 기존 테이블들을 유지하면서 새로운 기능 추가

-- 1. 기존 테이블들 삭제 (workers, schedules, schedule_workers, work_periods만)
DROP TABLE IF EXISTS work_periods CASCADE;
DROP TABLE IF EXISTS schedule_workers CASCADE;
DROP TABLE IF EXISTS schedules CASCADE;
DROP TABLE IF EXISTS workers CASCADE;

-- 2. Workers 테이블 생성 (새로운 구조)
...
-- 9. 기존 clients 테이블에 파일 관련 컬럼 추가
ALTER TABLE clients ADD COLUMN IF NOT EXISTS documents_folder_path TEXT; -- 문서 폴더 경로
ALTER TABLE clients ADD COLUMN IF NOT EXISTS contact_person TEXT; -- 담당자명 (기존 client_contacts와 중복 방지)
...
-- 12. User_Profiles 테이블 생성 (사용자 프로필 정보)
CREATE TABLE IF NOT EXISTS user_profiles (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  business_name TEXT, -- 사업체명
  business_number TEXT, -- 사업자등록번호
  business_address TEXT, -- 사업장 주소
  business_phone TEXT, -- 사업장 전화번호
  business_email TEXT, -- 사업장 이메일
  -- 파일 관련 컬럼들
  business_card_image_url TEXT, -- 명함 사진 URL
  business_card_image_path TEXT, -- 명함 사진 경로
  business_license_image_url TEXT, -- 사업자등록증 사진 URL
  business_license_image_path TEXT, -- 사업자등록증 사진 경로
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
COMMENT ON TABLE user_profiles IS '사용자 프로필 및 사업체 정보';
