...
-- This migration creates all the basic tables for the remit-planner application
...
-- Schedule-Worker relationship table
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_schedule_workers_updated_at') THEN
    CREATE TRIGGER update_schedule_workers_updated_at BEFORE UPDATE ON schedule_workers
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_work_periods_updated_at') THEN
